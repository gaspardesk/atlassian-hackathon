...
export async function checkIfSignedUp(user_domain) {
  try {
    const response = await fetch(
      `https://${user_domain}.us.gaspardesk.com/api/common/info/version`
    );
...
export async function send_message(message) {
  ...
  try {
    const django_response = await fetch(
      `https://5074-94-66-221-143.eu.ngrok.io/forge/message`,
      {
...
export async function send_confirmation(worker) {
  ...
  try {
    const django_response = await fetch(
      `https://5074-94-66-221-143.eu.ngrok.io/forge/intention/confirm`,
      {
        headers: build_headers(current_user.domain_name),
        method: "POST",
        body: JSON.stringify(message_body),
